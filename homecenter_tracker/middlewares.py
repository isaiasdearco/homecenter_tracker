...
class HomecenterTrackerSpiderMiddleware:
...
class HomecenterTrackerDownloaderMiddleware:
    # Not all methods need to be defined. If a method is not defined,
...
class ScrapeOpsFakeBrowserHeaderAgentMiddleware:
    ...
    def __init__(self, settings):
        self.scrapeops_api_key = settings.get("SCRAPEOPS_API_KEY")
        self.scrapeops_endpoint = settings.get(
            "SCRAPEOPS_FAKE_BROWSER_HEADERS_ENDPOINT",
...
class ProxyMiddleware:
    ...
    def __init__(self, settings):
        ...
        self.password = settings.get("PROXY_PASS")
        self.endpoint = settings.get("PROXY_ENDPOINT")
        self.port = settings.get("PROXY_PORT")

    def process_request(self, request, spider):
        print("************ NEW PROXY ATTACHED *******")
        user_credentials = "{user}:{password}".format(
            user=self.user, password=self.password
        )
        host = f"http://{self.user}:{self.password}@p.webshare.io:80"
